...
#define MIN_DUMP_TIME        (150*1000) // unit: us
...
#define PROC_RAW_FRZ "mt6883 mt6885 mt6873 mt6853 mt6893 mt6833 mt6877 mt6781 mt6855 mt6789 mt6835"
#define CCT_USING_BY_INDEX "mt6883 mt6885 mt6873 mt6853 mt6893 mt6779 mt6833 mt6877 mt6781 mt6855 mt6789 mt6835"

static MBOOL IS_PROC_RAW_FRZ = MFALSE;
static MBOOL IS_CCT_USING_BY_INDEX = MFALSE;

static char TARGET_BOARD_PLATFORM[PROPERTY_VALUE_MAX];
...
private:
    ...
    MINT32 changeRawType(const CCT_CAPTURE_IN& cap_in);
...
CamTool()
    ...
    , m_iFdOut(-1)
{
    property_get("ro.board.platform", TARGET_BOARD_PLATFORM, "0");
    CAMTOOL_LOGD("[%s] TARGET_BOARD_PLATFORM = %s", __FUNCTION__, TARGET_BOARD_PLATFORM);

    if( strstr(PROC_RAW_FRZ, TARGET_BOARD_PLATFORM) )
        IS_PROC_RAW_FRZ = MTRUE;
    if( strstr(CCT_USING_BY_INDEX, TARGET_BOARD_PLATFORM) )
        IS_CCT_USING_BY_INDEX = MTRUE;
}
...
void set_vendor_property(const char *name, const char *value)
{
    char tcmd[300];
    int retVal = 0;
    retVal = snprintf(tcmd, 300, "setprop %s %s", name, value);
    if (retVal < 0)
    {
...
MINT32 _find_file_latest(char *findfile, const char *folder, const char *pre , const char *mid, const char *post)
{
    ...
    while ((ptr=readdir(dir)) != NULL)
    {
             ...
             ( (strlen(filename)>=strlen(post)) && (0 == strcmp(filename+strlen(filename)-strlen(post), post)) )) )
        {
            ...
            char filePath[1024];
            int fileSize = 0;
            checkSnprintf = sprintf(filePath, "%s/%s", folder, filename); // do NOT use hard code for folder
            if(checkSnprintf < 0){
            ...
            if (((0 == findfile[0]) || (0 > strncmp(findfile, filename, FLEN))) && fileSize > 0)   // remember to also check fileSize > 0
            {
                strncpy(findfile, filename, FLEN);
            }
...
MINT32 _find_file_first(char *findfile, const char *folder, const char *pre , const char *mid, const char *post)
{
    CAMTOOL_LOGD("folder=%s, pre=%s, mid=%s, post=%s", folder, pre ? pre : "[NULL]", mid ? mid : "[NULL]", post ? post : "[NULL]");
    findfile[0] = 0;
    int checkSnprintf = 0;
...
MINT32 _cmd_capture(int &get_unpacked_raw, CT_OUT_FORMAT_T out_format, int wait_time)
{
    ...
    char srcPath[FLEN] = {'\0'};
    char dstPath[FLEN] = {'\0'};
    char dstDebugPath[FLEN] = {'\0'};
    // 3. CHECK
    switch(out_format){
    case CT_FORMAT_PURE:
        for(int i=0; i<wait_time; i++) {
            ret = _find_file_first(dumpFile, "/data/vendor/camera_dump", NULL, "-imgo", ".packed_word"); //system_call("test -e /data/vendor/camera_dump/*-imgo*.packed_word");
            if(!ret) {break;}
            ret = _find_file_first(dumpFile, "/data/vendor/camera_dump", NULL, "-imgo", ".raw"); //system_call("test -e /data/vendor/camera_dump/*-imgo*.raw");
            if(!ret)
            {
                get_unpacked_raw = 1;
...
MINT32 _cmd_preview(CT_PROFILE_MODE_T profile_mode, CT_OUT_FORMAT_T out_format, int dump_time, int wait_time)
{
    ...
    for(int i=0; i<wait_time; i++) {
        ...
        case CT_FORMAT_PROC:
            if(IS_PROC_RAW_FRZ)
                ret = _find_file_first(dumpFile, "/data/vendor/camera_dump", NULL, "-imgo", ".raw"); //system_call("test -e /data/vendor/camera_dump/*-imgo*.packed_word");
            else
                ret = _find_file_first(dumpFile, "/data/vendor/camera_dump", NULL, "-imgo", ".packed_word"); //system_call("test -e /data/vendor/camera_dump/*-imgo*.packed_word");
            break;
        case CT_FORMAT_PROC_LSC_16:
            ...
            break;
        case CT_FORMAT_JPEG:
            ret = _find_file_first(dumpFile, "/data/vendor/camera_dump", NULL, "-img3o", ".yv12");  //system_call("test -e /data/vendor/camera_dump/*.yv12");
            if(0 != ret) {
                CAMTOOL_LOGD("cannot find *-img3o*.yv12, try *-wdmao*.yv12 ,ret=%d", ret);
                ret = _find_file_first(dumpFile, "/data/vendor/camera_dump", NULL, "-wdmao", ".yv12");  //system_call("test -e /data/vendor/camera_dump/*.yv12");
            }
            break;
        }
    ...
    for(int i=0; i<wait_time; i++) {
        ...
        if(debgu_dump){
            checkSnprintf = snprintf(tcmd, 600, "cp %s %s", srcPath, dstDebugPath);
            if(checkSnprintf < 0){
        ...
        checkSnprintf = snprintf(tcmd, 600, "mv %s %s", srcPath, dstPath);
        if(checkSnprintf < 0){
            CAMTOOL_LOGE("[%s] snprintf return error code.", __FUNCTION__);
            goto FAILED;
        }
...
MINT32 _cmd_preview_recap(CT_PROFILE_MODE_T profile_mode, CT_OUT_FORMAT_T out_format, int dump_time = 500*1000)
{
    ...
    for(int i=1; (0!=ret) && (i<=recap_max); i++)
    {
        ...
        usleep(2000*1000);
        ret = _cmd_preview(profile_mode, out_format, MIN(dump_time, dump_time_max), 15);
    }
    return ret;
}
...
MINT32 captureCommand(CCT_CAPTURE_IN cap_in, int &unpacked_raw, int dump_time/* = -1*/)
{
    ...
    case CT_PROFILE_MODE_CAPTURE:
        ...
        case CT_FORMAT_PROC:
        case CT_FORMAT_PROC_LSC_16:
            ret = _cmd_preview_recap(cap_in.profile_mode, cap_in.out_format, (dump_time > MIN_DUMP_TIME) ? dump_time : MIN_DUMP_TIME);
            break;
        }
        break;
            ...
            if(( pipeOut = popen("echo /mnt/vendor/cct/pure__* | sed -r 's/^.+\\///' | sed 's/_/ /g;s/x/ /g;s/[^0-9 ]*//g;'", "r") ))
            {
                auto getValue = [] (int &value, FILE *fp) -> void
                {
                    ...
                    do
                    {
                        c = fgetc(fp);
                        if(ferror(fp)){
                            CAMTOOL_LOGE("[%s] fgetc return error code in getValue.", __FUNCTION__);
                            clearerr(fp);
                            break;
                        }
                        if( c >= '0' && c <= '9' )
...
MVOID FreePointersBeforeReturn(FILE*& inFPtr, FILE*& outFPtr, MUINT8*& cmdBuf, MUINT8*& cmdOutBuf)
{
    if(fclose(inFPtr)){}
    if(fclose(outFPtr)){}
    if(cmdBuf)
...
{    
    ...
    {
        checkSnprintf = snprintf(cct_sensor_index, 10, "%d", sensorIndex);
        CAMTOOL_LOGD("[%s] CCT connect by sensor index: %s", __FUNCTION__, cct_sensor_index);
    }
    else
    {
        checkSnprintf = snprintf(cct_sensor_index, 10, "%d", 1 << (sensorIndex-1));
        CAMTOOL_LOGD("[%s] CCT connect by device id: %s", __FUNCTION__, cct_sensor_index);
    }
    ...
    if(IS_CCT_USING_BY_INDEX)
    {
        char sensorName[PROPERTY_VALUE_MAX];
        ...
        if (strcmp(sensorName,"0"))
        {
            ...
            if(len < 1){
                CAMTOOL_LOGE("[%s] sensorName to short!", __FUNCTION__);
            }else{
                int sensorNum = sensorName[len-1] - '0'; // atoi
                sensorName[len-1] = '-';
                char sensorType[10];
                checkSnprintf = snprintf(sensorType, 9, "%s%d", sensorName, sensorNum-1);
                if(checkSnprintf < 0){
                    CAMTOOL_LOGE("[%s] snprintf return error code.", __FUNCTION__);
                }
...
changeProfileMode(CT_PROFILE_MODE_T profile_mode)
{
    ...
    switch(profile_mode){
        case CT_PROFILE_MODE_PREVIEW:
        case CT_PROFILE_MODE_CAPTURE:
            if(strcmp(sensorName, "back-0") && strcmp(sensorName, "front-0")) // if not Cam0 or Cam1, need to switch to Cam0 and then switch back
            {
                set_vendor_property("mtk.camera.switch.id.debug", "back-0");
...
MINT32
CamTool::
changeRawType(const CCT_CAPTURE_IN& cap_in) {
  char cmd[200] = {'\0'};
  CAMTOOL_LOGD("Format(%d), Mode(%d)", cap_in.out_format, cap_in.profile_mode);
  ...
  if (cap_in.out_format == CT_FORMAT_PROC_LSC_16 ||
    ...
    set_vendor_property("vendor.debug.camera.afraw", "1"); // 16-bit raw
  } else {
    set_vendor_property("vendor.debug.camera.cfg.ProcRaw", "0");
    set_vendor_property("vendor.debug.camera.afraw", "0");
  }
  system_call("am force-stop com.mediatek.camera");
  usleep(1000 * 1000);
  if (cap_in.out_format == CT_FORMAT_PROC_LSC_16 ||
      cap_in.out_format == CT_FORMAT_PROC)
    set_vendor_property("vendor.camsys.frz_disable", "1");  // disable FRZ
  else
    set_vendor_property("vendor.camsys.frz_disable", "0");
  ...
  //make FIFO of camtool side ready after restart
  if ( 0 != ReOpenFifo() ) {
    return -2;
  }
    ...
    while (feof(inFPtr) == 0 && stopFlag == MFALSE) {
        if ((err = getCctCommand(inFPtr, &cmdOp, &bufSize, (void **)&cmdBuf)) == 0) {
            ...
            if ( cmdOp == CT_CMD_OP_STOP_EXEC ) {
                ...
                if(cmdlen < 0){
                    CAMTOOL_LOGE("[%s] Negative index: cmdlen!", __FUNCTION__);
                    FreePointersBeforeReturn(inFPtr, outFPtr, cmdBuf, cmdOutBuf);
                    return -2;
                }
                ...
                if (IS_PROC_RAW_FRZ && cmdOp >= FT_CCT_OP_CAL_START && cmdOp < FT_CCT_OP_CAL_TYPE_MAX && cmdBuf) {
                    if (cmdOp == FT_CCT_OP_CAL_CDVT_CALIBRATION) {
                        ACDK_CDVT_SENSOR_CALIBRATION_INPUT_T* pCaliInput = reinterpret_cast<ACDK_CDVT_SENSOR_CALIBRATION_INPUT_T*>(cmdBuf);
                        MINT32 sensorMode = static_cast<MINT32>(pCaliInput->eSensorMode);
                        CCT_CAPTURE_IN cap_in = {};
                        cap_in.profile_mode = (sensorMode < CT_PROFILE_MODE_MAX) ? static_cast<CT_PROFILE_MODE_T>(sensorMode) : CT_PROFILE_MODE_VIDEO;
                        if (pCaliInput->eCalibrationItem == ACDK_CDVT_CALIBRATION_OB) {
                            cap_in.out_format = CT_FORMAT_PURE;
                        } else {
                            cap_in.out_format = CT_FORMAT_PROC_LSC_16;
                        }
                        err = changeRawType(cap_in);
                    } else if (cmdOp == FT_CCT_OP_CAL_CDVT_TEST) {
                        ACDK_CDVT_SENSOR_TEST_INPUT_T* pTestInput = reinterpret_cast<ACDK_CDVT_SENSOR_TEST_INPUT_T*>(cmdBuf);
                        MINT32 sensorMode = static_cast<MINT32>(pTestInput->eSensorMode);
                        CCT_CAPTURE_IN cap_in = {};
                        cap_in.profile_mode = (sensorMode < CT_PROFILE_MODE_MAX) ? static_cast<CT_PROFILE_MODE_T>(sensorMode) : CT_PROFILE_MODE_VIDEO;
                        cap_in.out_format = CT_FORMAT_PURE;
                        err = changeRawType(cap_in);
                    } else if (cmdOp == FT_CCT_OP_CAL_SHADING) {
                        ACDK_CCT_LSC_CAL_SET_STRUCT* pLSCCaliInput = reinterpret_cast<ACDK_CCT_LSC_CAL_SET_STRUCT*>(cmdBuf);
                        MINT32 sensorMode = static_cast<MINT32>(pLSCCaliInput->mode);
                        CCT_CAPTURE_IN cap_in = {};
                        cap_in.profile_mode = (sensorMode < CT_PROFILE_MODE_MAX) ? static_cast<CT_PROFILE_MODE_T>(sensorMode) : CT_PROFILE_MODE_VIDEO;
                        cap_in.out_format = CT_FORMAT_PROC_LSC_16;
                        err = changeRawType(cap_in);
...
int mainFunc(int argc, char *argv[])
{
    ...
    if (strncmp(argv[1], "camera-on", 9) == 0) {
        ...
        else
        {
            ...
            if(IS_PROC_RAW_FRZ)
            {
                set_vendor_property("vendor.debug.camera.dump.campipe", "1");
                set_vendor_property("vendor.debug.camera.remo.enable", "0"); //disable full size for pure raw capture
                set_vendor_property("vendor.debug.p2s.dsdn.threshold", "99999"); //disable DSDN for jpeg dump .yv12
                set_vendor_property("vendor.debug.fpipe.frame.setting", "1"); //for jpeg dump .yv12
                set_vendor_property("vendor.debug.fpipe.force.img3o", "1"); //for jpeg dump .yv12
            }
        ...
        if(IS_PROC_RAW_FRZ)
        {
            set_vendor_property("vendor.debug.camera.dump.campipe", "0");
            set_vendor_property("vendor.debug.camera.remo.enable", "1");
            set_vendor_property("vendor.debug.p2s.dsdn.threshold", "0");
            set_vendor_property("vendor.debug.fpipe.frame.setting", "0");
            set_vendor_property("vendor.debug.fpipe.force.img3o", "0");
        }
